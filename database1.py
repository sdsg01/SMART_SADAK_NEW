...
def get_latest_time(rfid_ID, scanner_ID):
    ...
    res = ref1.order_by_child('rfid').equal_to(rfid_ID).limit_to_last(1).get()
...
def get_count(start, end):
    count = 0
    star = time.strptime(start, "%H:%M:%S")
    s1 = datetime.timedelta(hours=star.tm_hour, minutes=star.tm_min, seconds=star.tm_sec).seconds
    en = time.strptime(end, "%H:%M:%S")
    e1 = datetime.timedelta(hours=en.tm_hour, minutes=en.tm_min, seconds=en.tm_sec).seconds
    ref1 = ref.child('Scan_Details_log')
    result = ref1.order_by_child('scanner_id').equal_to(2).get()
    for key, val in result.items():
        #print(val['time'])
        s = time.strptime(val['time'], "%H:%M:%S")
        s_hr = datetime.timedelta(hours=s.tm_hour, minutes=s.tm_min, seconds=s.tm_sec).seconds
        if(s_hr >s1 and s_hr<e1):
            count+=1
